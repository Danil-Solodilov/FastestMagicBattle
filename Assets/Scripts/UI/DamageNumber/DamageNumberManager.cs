...
public class DamageNumberManager : MonoBehaviour
{
    ...
    public void SpawnDamageNumber(Vector3 position, float amount, bool isCrit, bool isBurn, bool isFreeze, bool isStun)
    {
        DamageNumber dn = _pool.Get();
        dn.transform.position = position;
        dn.Setup(amount, _pool, isCrit, isBurn, isFreeze, isStun); // Передаем пул, чтобы цифра сама могла вернуться в него
    }
}
